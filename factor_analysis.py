...
def factor_catalog() -> list[dict[str, str]]:
    ...
    rows.extend(
        [
            ...
            _catalog_row("持有期时点评分", "同周期动量", "clip(Rh÷(2×近60日波动×√h),-1,1)×8", "当前与h日前收盘价、近60日日收益波动", "正收益为正面，但需按股票正常波动调整", "当前状态分最多±8分；历史验证只决定能否形成可操作方向", "收益或波动不可得时该周期不可用"),
            _catalog_row("持有期时点评分", "相对基准收益", "clip((R股票,h-R基准,h)÷(2×正常波动),-1,1)×7", "股票和对应基准同周期价格", "跑赢基准为正面", "当前状态分最多±7分；历史验证只决定能否形成可操作方向", "基准缺失时0分并降低完整度"),
            _catalog_row("持有期时点评分", "20日短期均值回归", "近10日收益相对自身近250日分布的z分数；z≥1扣分，z≤-1小幅加分", "至少120个交易日；只用于20日持有期", "极端追高偏负面，短期超跌小幅正面", "当前状态分-5至+3分；其他周期0分；历史验证只控制可操作性", "样本不足或非20日周期时0分"),
    ...
    rows.append(
        {
            ...
            "依据": (
                str(production_gate.get("reason") or "历史验证不足时不形成方向判断。")
                + " 未通过时仍展示当前状态强弱，但不产生买入仓位。"
            ),
        }
    )
